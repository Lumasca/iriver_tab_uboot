...
extern void print_pmicinfo(void); /* ktj */
...
int print_cpuinfo(void)
{
	...
	set_speed = 120000;
#elif defined(CONFIG_CLK_400_200_166_133) || defined(CONFIG_CLK_400_100_166_133)  /* ktj add */
	set_speed = 40000;
#elif defined(CONFIG_CLK_200_200_166_133)   /* ktj add */
	set_speed = 20000;
#elif defined(CONFIG_CLK_100_100_166_133)   /* ktj add */
	set_speed = 10000;
#else
	set_speed = 100000;
	...
	#ifdef CONFIG_MCP_SINGLE
	add_board_info(1,"CPU:  S5PV210@%ldMHz(%s)\n", get_ARMCLK()/1000000, ((result_set == 1) ? "OK" : "FAIL"));
	#else
	add_board_info(1,"\nCPU:  S5PC110@%ldMHz(%s)\n", get_ARMCLK()/1000000, ((result_set == 1) ? "OK" : "FAIL"));
	#endif
	add_board_info(1,"        APLL = %ldMHz, HclkMsys = %ldMHz, PclkMsys = %ldMHz\n",
			get_FCLK()/1000000, get_HCLK()/1000000, get_PCLK()/1000000);
	#if 1
	add_board_info(1,"	MPLL = %ldMHz, EPLL = %ldMHz\n",
			get_MPLL_CLK()/1000000, get_PLLCLK(EPLL)/1000000);
	add_board_info(1,"		       HclkDsys = %ldMHz, PclkDsys = %ldMHz\n",
			get_HCLKD()/1000000, get_PCLKD()/1000000);
	add_board_info(1,"		       HclkPsys = %ldMHz, PclkPsys = %ldMHz\n",
			get_HCLKP()/1000000, get_PCLKP()/1000000);
	add_board_info(1,"		       SCLKA2M  = %ldMHz\n", get_SCLKA2M()/1000000);
	#endif
	add_board_info(1,"Serial = CLKUART ");
#endif
