...
/* ktj modified partion for iriver MX100 */
#if defined(CFG_MOVI_PART) 
#if (MOVI_PART_TYPE == MOVI_PART_FAT)
...
#elif (MOVI_PART_TYPE == MOVI_PART_FAT_DATA) 
fastboot_ptentry ptable_default[] =
{
	{
		.name     = "bootloader",
...
#elif (MOVI_PART_TYPE == MOVI_PART_FAT_DATA_CACHE) 
fastboot_ptentry ptable_default[] =
{
	{
		.name     = "bootloader",
...
fastboot_ptentry ptable_default[] =
{
    /* onenand */
	{
		.name     = "bootloader",
		.start    = 0x0,
		.length   = 0xC0000,       // 768Kb 
		.flags    = 0
	},
	#ifdef USE_VENDOR_MISC_PART
	{
		.name     = "misc",		   // zsani2: use vendor partition
		.start    = 0xC0000,
...
void fboot_usb_int_bulkout(u32 fifo_cnt_byte)
{
	...
	*/
	...
	if (fastboot_interface && fastboot_interface->rx_handler) {
		...
		if (memcmp(fastboot_bulk_fifo, "flash:", 6) == 0  && !strcmp("system", fastboot_bulk_fifo + 6))
		{
			char *response = "OKAY";
			printf("system.img transper ok\n");
...
int fastboot_preboot(void)
{
    ...
    /* avoid entering fastboot mode, ktj */
