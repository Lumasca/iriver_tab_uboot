// 
//jpeg decode module for displaying the boot logo
//  2008.11.18  by jhlim.



#define D_FUNCPTR(type, name, args)    type (*name) args

typedef D_FUNCPTR(void,tfp_putpixel_lc,(void *dec, int xx, int yy, int y, int cb, int cr));
...
#define _MID(x,y,z)   _MAX((x), _MIN((y), (z)))
...
static unsigned long next_bit(struct DECODER_DATA *dec)
{
//	long seg_len;
...
*/
static int get_bits(struct DECODER_DATA *dec, int category)
{
	unsigned int i;
	int result = 0;
...
*/
static int huffman_decode(struct DECODER_DATA *dec, HUFFMAN_NODE *node)
{
	int i, j, found = -1;
	long val;
...
*/
static int read_dht(struct DECODER_DATA *dec)
{
	int bit_len[16];
...
*/
static int read_sof0(struct DECODER_DATA *dec)
{
	unsigned char comps;
	int i;
...
*/
static int read_dqt(struct DECODER_DATA *dec)
{
...
*/
static int read_sos(struct DECODER_DATA *dec)
{
	int i;
...
*/
static int read_app0(struct DECODER_DATA *dec)
{
	int skip;
...
*/
static int decode_block(struct DECODER_DATA *dec, int *dat, int dc, int ac, int qt, int *old_dc)
{
//	int temp[64];
...
int decode_memory_jpghdr(struct DECODER_DATA *dec,void *data) {
	int i;
	memset(dec, 0, sizeof(struct DECODER_DATA));
...
void decode_memory_jpg(struct DECODER_DATA *dec) {
		...
		case 4:
			...
			*  vertically), and luminance for every pixel.
				*/
				...
				for (i=0; i<8; i++) {
					for (j=0; j<8; j++) {
						dec->putpixel_lc(dec->video_buffer, x + j, y + i,
						y1[(i << 3) | j],
						cb[((i >> 1) << 3) | (j >> 1)],
						cr[((i >> 1) << 3) | (j >> 1)]);
					}
				...
				for (i=0; i<8; i++) for (j=0; j<8; j++) {
					dec->putpixel_lc(dec->video_buffer, x + 8 + j, y + i,
					y2[(i << 3) | j],
					cb[((i >> 1) << 3) | ((j >> 1) + 4)],
					cr[((i >> 1) << 3) | ((j >> 1) + 4)]);
				}
				...
				for (i=0; i<8; i++) for (j=0; j<8; j++) {
					dec->putpixel_lc(dec->video_buffer, x + j, y + 8 + i,
					y3[(i << 3) | j],
					cb[(((i >> 1) + 4) << 3) | (j >> 1)],
					cr[(((i >> 1) + 4) << 3) | (j >> 1)]);
				}
				...
				for (i=0; i<8; i++) for (j=0; j<8; j++)  {
					dec->putpixel_lc(dec->video_buffer, x + 8 + j, y + 8 + i,
					y4[(i << 3) | j],
					cb[(((i >> 1) + 4) << 3) | ((j >> 1) + 4)],
					cr[(((i >> 1) + 4) << 3) | ((j >> 1) + 4)]);
				}
	  ...
	  *  have a grayscaled picture.
          */
		  do {
			  decode_block(dec, y1, dec->dc_y, dec->ac_y, dec->quant_y, &dc_y);
			  for (i=0; i<8; i++) for (j=0; j<8; j++) {
				  hue = _MID(0, y1[(i << 3) | j], 255);
				  dec->putpixel_lc(dec->video_buffer,x + j, y + i, hue, hue, hue);
				  //				   putpixel(video_buffer, x + j, y + i, makecol24(hue, hue, hue));
			  }
			  x += 8;
			  if (x >= dec->width) {
				  x = 0;
...
*/
static struct DECODER_DATA g_jpegdec;

void load_memory_jpg(void *data,tfp_putpixel_lc putpixel,char *video_buffer,int scr_width,int scr_height)
{

	
	decode_memory_jpghdr(&g_jpegdec,data);

	g_jpegdec.putpixel_lc = putpixel;
	g_jpegdec.video_buffer = video_buffer;

	g_jpegdec.scr_width = scr_width;
	g_jpegdec.scr_height = scr_height;
	
	decode_memory_jpg(&g_jpegdec);
