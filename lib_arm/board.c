...
static int display_dram_config (void)
{
	...
	{
		char dram_size[32];
		sprint_size(dram_size,size);		
		add_board_info(1,"DRAM: %s ",dram_size);
	}
	#endif
...
static void display_flash_config (ulong size)
{
	...
	{
		char _size[32];
		sprint_size(_size,size);		
		add_board_info(1,"Flash: %s ",_size);
	}	
	#endif
}
#endif /* CFG_NO_FLASH */
...
extern void mx100_pre_init(void);
...
void start_armboot (void)
{
	...
	#ifdef CONFIG_MX100 /* ktj */
    	gpio_init();  // mov here.
	#endif

	mx100_pre_init();  /* JHLIM 2011.05.15  */
...
//******************************//
	...
	if (INF_REG3_REG == BOOT_ONENAND) {
		...
		if (mmc_exist != 0)
		{
			puts ("0 MB\n");
			#ifdef DISPLAY_BOARD_INFO
			add_board_info (0,"mmc error :0 MB\n");
			#endif				
		}
	#endif
...
//[yoon20110624]Turn off the power if do not recognize the MoviNAND.
	if (mmc_exist != 0)
	{
		puts ("### MoviNAND Error ### Power off ###\n");
		power_off();
	}
