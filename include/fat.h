...
#define FAT_DPRINT(args...) do { printf("[%s:%d]:", __FUNCTION__,__LINE__);    printf(args); } while (0)
...
#ifdef SD_DATA_COPY_SLICE  /*2011.03.30 JHLIM */
struct ts_data_slice_copy
{
...
typedef struct {
	...
	#ifdef SD_DATA_COPY_SLICE  /*2011.03.30 JHLIM */
	struct ts_data_slice_copy data_slice_copy;	
	int (*callback_write_slice) (struct ts_data_slice_copy *ps_dsc,unsigned char *buffer,int size);
	#endif
} fsdata;
